...
public class CoStar extends JDart {
	 ...
	 *            the JPF config
	 * @param initLogging
	 *            whether or not to initialize the logging system.
	 */
	public CoStar(Config conf, boolean initLogging) {
	...
	public ConcolicExplorer run() {
		...
		SimpleProfiler.start("JPF-boot"); // is stopped upon searchStarted in
											// ConcolicListener
		jpf.run();
			...
			for (CompletedAnalysis ca : e.getValue()) {
				...
				if (config.getBoolean("jdart.tree.json.print")) {
					ca.getConstraintsTree().toJson(
							config.getProperty("jdart.tree.json.dir") + "/" + jpfConf.getProperty("jpf.app") + ".json");
				}
				...
				logger.info("# of valuations (OK+ERR): " + (ca.getConstraintsTree().getCoveredPaths().size()
						+ ca.getConstraintsTree().getErrorPaths().size()));
				logger.info("");
